...
// int cache_access(cache_line **cache_sets, int s, int E, int b, int v, int
// bytes, int *hits, int *misses, int *evictions, unsigned long addr, char type)
// {
// 	/* 缓存模拟核心逻辑 */
// 	int hit_miss = 0, evictions_or_not = 0;
// 	char pre[20];
// 	/* 如果被 datalab 虐过,那下面的 << >> 应该不会陌生 */
// 	unsigned long tag = addr >> (b + s), sets = ((addr << (64 - b - s)) >>
// (64 - s)); 	cache_line *set = cache_sets[sets];
// 	/* 尝试读取 */
...
// 	int s, E, b, bytes, has_opt = 0, hits = 0, misses = 0, evictions = 0, v
// = 0; 	char *t; 	char input, type; 	FILE *fp; 	cache_line **cache;
// 	/* 解析命令行参数 */
...
// 			{
// 				/* 得到详细参数,进入缓存模拟核心逻辑 */
// 				cache_access(cache, s, E, b, v, bytes, &hits, &misses,
// &evictions, address, type);
// 			}
// 		}
// 		fclose(fp);
// 	}
